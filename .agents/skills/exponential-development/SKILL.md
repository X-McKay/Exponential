...
3. Preserve existing `.env`, SQLite databases, settings files, and uncommitted work. Normal startup creates a neutral workspace. Run `bun run demo:reset` only when the user explicitly requests disposable sample data.
...
7. Before handing work back, run `bun run check`, `bun run docs:check`, `bun run skills:check`, and `git diff --check`. Run `bun run test:e2e` for cross-layer or release-facing changes.
