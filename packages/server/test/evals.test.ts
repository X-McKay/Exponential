...
const AUDIT_REPLY = JSON.stringify({
  ...
  attention: true,
  body: "## Recall gap\nClause recall is 86% against an 88% base gate on MS-21. Coverage is 74% on clause-extractor. Made-up figure: 123456.",
...
describe("rules", () => {
  test("grounding compares cited figures and ids against the briefing", () => {
    const g = ungroundedTokens("Recall is 86% on MS-21; coverage 74%; invented 123456 and R9.", "MS-21 clause recall 86 base 88; coverage 74");
  ...
  test("a run gets format, grounding, and proposals_valid scores; a failed run scores 0 on format", async () => {
    ...
    const run = await runAgent(db, fakeLlm({ agent: () => AUDIT_REPLY }), { agentId: "audie", proj: "clauses" }, NOW);
    ...
    const failed = await runAgent(db, fakeLlm({ agent: () => "nonsense" }), { agentId: "audie", proj: "clauses" }, NOW);
...
describe("judge", () => {
  test("stores the four dimensions, a weighted overall, and expectations for benchmark runs; skips failed runs", async () => {
    ...
    const run = await runAgent(db, llm, { agentId: "audie", proj: "clauses" }, NOW);
    ...
    const failed = await runAgent(db, fakeLlm({ agent: () => "nope" }), { agentId: "audie", proj: "clauses" }, NOW);
  ...
  test("the judge route and auto-judge grade through the API", async () => {
    ...
    const run = (await call<AgentRun>("POST", routes.agentRuns("audie"), { proj: "clauses" })).body;
...
describe("benchmark", () => {
  test("runs each case for one agent, judges expectations, and the scorecard shows the batch", async () => {
    ...
    expect(runs.map((r) => r.benchmark)).toEqual(["audie-clauses-audit", "audie-search-habits", "audie-triage-readiness"]);
    const state = (await call<AppState>("GET", routes.state())).body;
    ...
    expect(card.benchmarks[0]).toMatchObject({ n: 3, failed: 0, promptVersion: promptVersion("audit"), model: "fake" });
    // 1 of 4 expectations met on the first case, 1 of 3 on the second and third.
    expect(card.benchmarks[0]?.expectations).toBeCloseTo((0.25 + 1 / 3 + 1 / 3) / 3, 5);
...
describe("workspace conversation", () => {
  test("the briefing covers every project; an answer stores a run of Ask with links and proposals", async () => {
    ...
    expect(brief).toContain("## Contract Clause Review (id: clauses, PRJ-7)");
    expect(brief).toContain("R1 Shadow mode (Oct, Blocked, 1/4)");
    expect(brief).toContain("Service level agreement (id: sla, Missing)");
    ...
    const llm = fakeLlm({
      chat: (m) => {
        ...
        return JSON.stringify({
          ...
          links: [{ label: "IMA roadmap", proj: "clauses", tab: "roadmap" }, { label: "bogus", proj: "nope", tab: "value" }],
          proposals: [{ type: "calendar_event", proj: "clauses", date: "2026-09-16", text: "Recall gate decision", sub: null, tab: "value", rationale: "Closest fix." }],
        });
      },
    });
    const reply = await askWorkspace(db, llm, { messages: [{ role: "user", content: "Why is R1 blocked?" }], proj: "clauses" }, NOW);
    expect(reply.answer).toContain("86%");
    expect(reply.links).toEqual([{ label: "IMA roadmap", proj: "clauses", tab: "roadmap" }]);
    ...
    expect(run).toMatchObject({ agentId: "ask", proj: "clauses", state: "done", instruction: "Why is R1 blocked?" });
