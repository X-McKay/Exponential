...
export function TargetsEditor({ targets, onSave, onClose }: { targets: ImpactPair; onSave: (t: ImpactPair) => Promise<unknown>; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Lbl htmlFor="vf-targetseditor-fte-reduction-target">FTE reduction target (%)</Lbl>
      <input id="vf-targetseditor-fte-reduction-target" type="number" min={0} max={100} style={{ ...inpStyle, borderColor: pctOk(t.fte) ? C.line2 : C.badLine2 }} value={t.fte} onChange={(e) => setT((x) => ({ ...x, fte: num(e.target.value) }))} />
      <Lbl htmlFor="vf-targetseditor-time-to-onboard-reduction-target">Time-to-onboard reduction target (%)</Lbl>
      <input id="vf-targetseditor-time-to-onboard-reduction-target" type="number" min={0} max={100} style={{ ...inpStyle, borderColor: pctOk(t.time) ? C.line2 : C.badLine2 }} value={t.time} onChange={(e) => setT((x) => ({ ...x, time: num(e.target.value) }))} />
